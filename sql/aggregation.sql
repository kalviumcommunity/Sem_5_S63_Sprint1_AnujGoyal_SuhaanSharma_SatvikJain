...
SELECT 
    ...
    ROUND(100.0 * SUM(CASE WHEN LOWER(s.completion_status) = 'completed' THEN 1 ELSE 0 END) / NULLIF(COUNT(s.student_id), 0), 2) AS completion_rate_pct,
...
HAVING AVG(bf.quiz_average) < 80.0 OR SUM(CASE WHEN LOWER(bf.dropout_risk_level) IN ('high', 'critical') THEN 1 ELSE 0 END) > 0
...
SELECT 
    ...
    ROUND(100.0 * SUM(q.passed) / NULLIF(COUNT(q.quiz_attempt_id), 0), 2) AS quiz_pass_rate_pct,
