...
SELECT 
    ...
    ROW_NUMBER() OVER (
        PARTITION BY s.target_course_id 
        ORDER BY COALESCE(bf.engagement_score, 0.0) DESC, s.student_id ASC
    ) AS course_engagement_rank,
    RANK() OVER (
        PARTITION BY s.target_course_id 
        ORDER BY COALESCE(bf.quiz_average, 0.0) DESC
    ) AS course_quiz_rank,
    ROUND(
        AVG(COALESCE(bf.engagement_score, 0.0)) OVER (PARTITION BY s.target_course_id), 
        2
...
ORDER BY s.target_course_id, course_engagement_rank;
...
SELECT 
    ...
    ROUND(
        duration_minutes - COALESCE(
            LAG(duration_minutes, 1) OVER (PARTITION BY student_id ORDER BY session_start),
            duration_minutes
        ), 
        2
...
SELECT 
    ...
    ROUND(
        score_percentage - COALESCE(
            LAG(score_percentage, 1) OVER (PARTITION BY student_id, quiz_id ORDER BY attempt_number),
            score_percentage
        ),
        2
...
SELECT 
    ...
    COALESCE(bf.quiz_average, 0.0) AS quiz_average,
    RANK() OVER (
        ORDER BY COALESCE(bf.engagement_score, 0.0) DESC
    ) AS overall_engagement_rank,
    DENSE_RANK() OVER (
        ORDER BY COALESCE(bf.quiz_average, 0.0) DESC
    ) AS overall_quiz_dense_rank,
    ROUND(
        AVG(COALESCE(bf.engagement_score, 0.0)) OVER (), 
        2
